...
// PDF oluşturmak için Puppeteer kullanacağız (tam Türkçe destekli)
async function createPdf(data, saatDetaylar) {
  // Basit HTML şablonu oluştur
  const html = `
<!DOCTYPE html>
<html lang="tr">
<head>
...
<body>
    ...
    <table>
        ...
        <tbody>
            ${saatDetaylar.map(d => `
            <tr><td>${d.saat}</td><td>${d.detay}</td></tr>`).join("")}
        </tbody>
    </table>
    <footer>
  ...
  await page.pdf({ path: pdfPath, format: "A4", printBackground: true, margin: { top: "40px", bottom: "40px", left: "30px", right: "30px" } });
  await browser.close();
  return pdfPath;
}
